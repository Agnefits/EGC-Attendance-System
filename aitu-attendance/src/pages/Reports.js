...
import { attendanceService, employeesService, leavesService, structureService } from '../services';
...
function Reports({ t, lang, user }) {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [attData, empData, deptData, leaveData, colData] = await Promise.all([
  ...
  const userAtt = attendance.filter(a => {
    if (user.role === 'head') { const e = employees.find(x => x.id === a.employeeId); return e && (e.departmentId === user.departmentId || e.department === user.department); }
    return true;
  });
  const userLeaves = leaves.filter(l => {
    if (user.role === 'head') { const e = employees.find(x => x.id === l.employeeId); return e && (e.departmentId === user.departmentId || e.department === user.department); }
    return true;
  });

  const total = userAtt.length;
  const present = userAtt.filter(a => a.status === 'present' || a.status === 'left').length;
  const absent = userAtt.filter(a => a.status === 'absent').length;
  const late = userAtt.filter(a => a.status === 'late').length;
  const pct = total ? Math.round(present / total * 100) : 0;

  const deptStats = departments.filter(d => user.role === 'head' ? (d.id === user.departmentId || d.name === user.department) : true).map(dept => {
    const emps = employees.filter(e => e.departmentId === dept.id || e.department === dept.name);
    const atts = userAtt.filter(a => emps.find(e => e.id === a.employeeId || e.name === a.employeeName));
  ...
  const leaveStats = LEAVE_TYPES.map(lt => ({
    ...lt,
    total: userLeaves.filter(l => l.type === lt.id).length,
    approved: userLeaves.filter(l => l.type === lt.id && l.status === 'approved').length,
    pending: userLeaves.filter(l => l.type === lt.id && l.status === 'pending').length,
    rejected: userLeaves.filter(l => l.type === lt.id && l.status === 'rejected').length,
  }));
      ...
      <div style={{ display: 'flex', gap: '4px', background: 'white', padding: '4px', borderRadius: '12px', border: '1px solid #E8EDF5', width: 'fit-content', marginBottom: '20px', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => { setActiveTab(tab.id); setViewMode('table'); }}
            style={{
              display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 20px', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'Cairo', transition: 'all .18s',
              ...
              {deptStats.map(d => {
                ...
                return (
                  <div key={d.id}
                    ...
                    style={{
                      display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '14px 10px', borderRadius: '14px',
          ...
          <div style={{ display: 'flex', gap: '3px', background: 'white', padding: '3px', borderRadius: '10px', border: '1px solid #E8EDF5', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
            {[{ id: 'table', icon: '📋', l: { ar: 'جدول', en: 'Table' } }, { id: 'cards', icon: '🃏', l: { ar: 'بطاقات', en: 'Cards' } }].map(v => (
              <button key={v.id} onClick={() => setViewMode(v.id)}
                style={{
                  display: 'flex', alignItems: 'center', gap: '5px', padding: '7px 14px', border: 'none', borderRadius: '8px', fontSize: '12px', fontWeight: '700', cursor: 'pointer', fontFamily: 'Cairo', transition: 'all .18s',
          ...
          {leaveStats.map(lt => {
            ...
            return (
              <div key={lt.id}
                ...
                style={{
                  ...card, padding: '16px', transition: 'all .2s', opacity: lt.total === 0 ? .5 : 1, textAlign: 'center', cursor: 'pointer', width: '170px', flexShrink: 0,
