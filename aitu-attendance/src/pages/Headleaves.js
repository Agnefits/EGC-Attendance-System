...
import { EMPLOYEES, DEPARTMENTS, LEAVES } from '../data';
...
export default function HeadLeaves({ lang, user }) {
  ...
  const [leaves,     setLeaves]     = useState(LEAVES.filter(l=>empIds.includes(l.employeeId)));
  const [activeTab,  setActiveTab]  = useState('pending');
  const [modal,      setModal]      = useState(null); // selected leave
  ...
  function approve(id) {
    ...
    showToast(lang==='ar'?'✓ تمت الموافقة على الأجازة':'✓ Leave approved');
    closeModal();
  }

  function reject() {
    if (!rejectNote.trim()) return;
    setLeaves(p=>p.map(l=>l.id===modal.id?{...l,status:'rejected',rejectNote}:l));
    showToast(lang==='ar'?'تم رفض الأجازة':'Leave rejected','error');
    closeModal();
  }
