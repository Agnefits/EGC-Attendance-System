...
import { EMPLOYEES, DEPARTMENTS, ATTENDANCE } from '../data';
...
function EmployeeLeaves({ lang, user }) {
  ...
  const [requests, setRequests] = useState([
    { id:1, type:'annual', from:'2026-05-25', to:'2026-05-29', days:5, reason:'إجازة سنوية', status:'pending' },
    { id:2, type:'sick',   from:'2026-04-02', to:'2026-04-03', days:2, reason:'مرضية',       status:'approved' },
    { id:3, type:'urgent', from:'2026-06-15', to:'2026-06-15', days:1, reason:'ظرف طارئ',    status:'pending' },
  ]);
  ...
  function submitRequest() {
    ...
    if (selectedType?.id==='maternity') {
      if (!maternityMode) { setFormError(lang==='ar'?'اختر طريقة احتساب الإجازة':'Select maternity calculation method'); return; }
      if (maternityMode==='birth'&&!birthDate) { setFormError(lang==='ar'?'أدخل تاريخ الولادة':'Enter birth date'); return; }
      if (maternityMode==='ninth'&&!ninthDate) { setFormError(lang==='ar'?'أدخل بداية الشهر التاسع':'Enter 9th month start date'); return; }
      if (!form.reason.trim()) { setFormError(lang==='ar'?'اكتب السبب':'Enter reason'); return; }
      const dates = computeMaternityDates();
      if (!dates) return;
      setRequests(p=>[...p,{ id:Date.now(), type:'maternity', from:dates.from, to:dates.to, days:120, reason:form.reason.trim(), status:'pending', maternityMode }]);
      showToast(lang==='ar'?'✓ تم إرسال طلب إجازة الوضع':'✓ Maternity leave submitted');
      setModalOpen(false);
      return;
    }

    if (!form.from||!form.to)    { setFormError(lang==='ar'?'اختر التاريخ من وإلى':'Select from and to dates'); return; }
    if (!form.reason.trim())     { setFormError(lang==='ar'?'اكتب سبب الإجازة':'Enter leave reason'); return; }
    ...
    // Check balance for types with limit
    if (lt.days>0) {
      const rem = (lt.days - usedDays(lt.id));
      if (days > rem) {
        setFormError(lang==='ar'?`الرصيد المتبقي ${rem} يوم فقط`:`Remaining balance: ${rem} days`);
        return;
      }
    }

    if (editId) {
      setRequests(p=>p.map(r=>r.id===editId?{...r,from:form.from,to:form.to,days,reason:form.reason.trim()}:r));
      showToast(lang==='ar'?'✓ تم تعديل الطلب':'✓ Request updated');
    } else {
      setRequests(p=>[...p,{ id:Date.now(), type:lt.id, from:form.from, to:form.to, days, reason:form.reason.trim(), status:'pending' }]);
      showToast(lang==='ar'?'✓ تم إرسال طلب الإجازة':'✓ Leave request submitted');
    }
    setModalOpen(false);
  }
                            ...
                            {weekDays.map(d=>{
                              ...
                              return(
                                <button key={d} type="button"
                                  ...
                                  style={{padding:'8px 12px',border:`1.5px solid ${isSel?'#B45309':'#E2E8F0'}`,borderRadius:'10px',
