...
import { EMPLOYEES, LEAVES, DEPARTMENTS, COLLEGES } from '../data';
...
export default function Leaves({ t, lang, user }) {
  ...
  const ALL_DEPTS = schedEmpType==='academic'
    ? DEPARTMENTS.filter(d=>d.collegeId===schedCollegeId||!schedCollegeId)
    : ADMIN_DEPTS;
  ...
  function saveSchedule(){
    ...
    const affected=schedEmpIds.length>0?schedEmpIds.map(id=>EMPLOYEES.find(e=>e.id===id)).filter(Boolean):schedType==='dept'?EMPLOYEES.filter(e=>e.departmentId===schedDeptId):[];
    setSchedules(p=>[{id:Date.now(),targetLabel,timeMode,checkIn:timeMode==='fixed'?schedForm.checkIn:null,checkOut:timeMode==='fixed'?schedForm.checkOut:null,hoursPerDay:timeMode==='fixed'?computeHours(schedForm.checkIn,schedForm.checkOut):schedForm.flexHours,daysPerWeek:schedForm.daysPerWeek,affected},...p]);
  ...
  /* Leave operations */
  const myEmpId   = user.employeeId||'EMP003';
  const myLeaves  = leaves.filter(l=> user.role==='admin'?true:l.employeeId===myEmpId);
  const filtered  = activeTab==='all'?myLeaves:myLeaves.filter(l=>l.status===activeTab);
  const pending   = myLeaves.filter(l=>l.status==='pending').length;
  const approved  = myLeaves.filter(l=>l.status==='approved').length;

  function approveLeave(id){ setLeaves(p=>p.map(l=>l.id===id?{...l,status:'approved'}:l)); }
  function rejectLeave(id){  setLeaves(p=>p.map(l=>l.id===id?{...l,status:'rejected'}:l)); }
  ...
  function submitGrant(){
    ...
    else if(grantForm.scope==='dept'){if(!grantForm.deptIds.length){setGrantError(lang==='ar'?'اختر قسماً':'Select dept');return;}grantForm.deptIds.forEach(dId=>{const sel=grantForm.deptSelections[dId];if(!sel||sel==='all'){EMPLOYEES.filter(e=>e.departmentId===dId).forEach(e=>{if(!targets.includes(e.id))targets.push(e.id);});}else sel.forEach(id=>{if(!targets.includes(id))targets.push(id);});});}
    else targets=EMPLOYEES.map(e=>e.id);
  ...
  function exportExcel(){
    const rows=[[lang==='ar'?'الاسم':'Name',lang==='ar'?'النوع':'Type',lang==='ar'?'من':'From',lang==='ar'?'إلى':'To',lang==='ar'?'الأيام':'Days',lang==='ar'?'الحالة':'Status'],...filtered.map(l=>{const e=EMPLOYEES.find(x=>x.id===l.employeeId);const lt=LEAVE_TYPES.find(x=>x.id===l.type);return[lang==='en'?e?.nameEn:e?.name,lt?.label[lang],l.from,l.to,l.days,STATUS[l.status]?.label[lang]];})];
  ...
  /* ─── Scope depts logic ─── */
  const grantDepts = grantForm.collegeId==='admin' ? ADMIN_DEPTS : grantForm.collegeId ? DEPARTMENTS.filter(d=>d.collegeId===grantForm.collegeId) : [];
        ...
        <div style={{display:'flex',gap:'4px',background:'white',padding:'5px',borderRadius:'14px',border:'1px solid #E8EDF5',width:'fit-content',marginBottom:'20px',boxShadow:'0 1px 4px rgba(0,0,0,.04)'}}>
          {[
            ...
            <button key={tab.id} onClick={()=>setMainTab(tab.id)}
              style={{padding:'10px 22px',border:'none',borderRadius:'10px',fontSize:'14px',fontWeight:'700',cursor:'pointer',fontFamily:'Cairo',transition:'all .18s',
          ...
          <div style={{display:'flex',borderBottom:'1px solid #F1F5F9'}}>
            {[
              ...
              <button key={tab.id} onClick={()=>setActiveTab(tab.id)}
                style={{flex:1,padding:'13px 10px',border:'none',cursor:'pointer',fontFamily:'Cairo',fontSize:'14px',fontWeight:activeTab===tab.id?'800':'500',
                ...
                {filtered.length===0?(
                  ...
                  return(
                    <tr key={l.id} style={{background:idx%2===0?'white':'#FAFBFC',transition:'background .15s'}}
                      ...
                      <td style={tdS({fontWeight:'800',color:'#1565C0',fontSize:'16px'})}>{l.days}</td>
                ...
                {[
                  ...
                  return(
                    <button key={s.id} onClick={()=>{setSchedType(s.id);setSchedDeptId('');setSchedCollegeId('');setSchedEmpIds([]);}}
                      style={{padding:'9px 6px',borderRadius:'9px',cursor:'pointer',fontFamily:'Cairo',fontSize:'12px',fontWeight:'700',textAlign:'center',
                    ...
                    {[
                      ...
                      return(
                        <button key={t.id} onClick={()=>{setSchedEmpType(t.id);setSchedCollegeId('');setSchedDeptId('');setSchedEmpIds([]);}}
                          style={{padding:'9px',borderRadius:'9px',cursor:'pointer',fontFamily:'Cairo',fontSize:'13px',fontWeight:'700',
                            border:`1.5px solid ${sel?'#1565C0':'#E2E8F0'}`,
                            background:sel?'#1565C0':'white',
                            color:sel?'white':'#475569',
                            transition:'all .15s'}}>
                      ...
                      {COLLEGES.map(c=>{
                        ...
                        return(
                          <button key={c.id} onClick={()=>{setSchedCollegeId(c.id);setSchedDeptId('');setSchedEmpIds([]);}}
                            style={{padding:'7px 14px',borderRadius:'8px',cursor:'pointer',fontFamily:'Cairo',fontSize:'12px',fontWeight:'700',
                              border:`1.5px solid ${sel?'#1565C0':'#E2E8F0'}`,
                              background:sel?'#1565C0':'white',
                ...
                {[
                  ...
                  return(
                    <button key={m.id} onClick={()=>setTimeMode(m.id)}
                      style={{padding:'12px 10px',borderRadius:'11px',cursor:'pointer',fontFamily:'Cairo',textAlign:'center',
                        border:`1.5px solid ${sel?'#1565C0':'#E2E8F0'}`,
              ...
              <div style={{display:'flex',gap:'6px'}}>
                {[5,6].map(n=>(
                  <button key={n} onClick={()=>setSchedForm(p=>({...p,daysPerWeek:n}))}
                    style={{padding:'8px 18px',borderRadius:'8px',cursor:'pointer',fontFamily:'Cairo',fontSize:'13px',fontWeight:'700',
              ...
              {schedules.length===0?(
                ...
                <div key={s.id} style={{background:'#F8FAFC',borderRadius:'12px',padding:'14px 16px',marginBottom:'10px',border:'1px solid #E8EDF5',transition:'all .18s'}}
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'8px'}}>
                    <span style={{fontSize:'14px',fontWeight:'700',color:'#0F172A'}}>{s.targetLabel[lang]}</span>
                    <span style={{background:s.timeMode==='fixed'?'#DBEAFE':s.timeMode==='open'?'#ECFEFF':'#EDE9FE',color:s.timeMode==='fixed'?'#1565C0':s.timeMode==='open'?'#0891B2':'#6B21A8',border:`1px solid ${s.timeMode==='fixed'?'#BFDBFE':s.timeMode==='open'?'#A5F3FC':'#DDD6FE'}`,padding:'3px 9px',borderRadius:'999px',fontSize:'11px',fontWeight:'700'}}>
                      {s.timeMode==='open'?(lang==='ar'?'ساعات مرنة':'Flexible Hours'):s.timeMode==='fixed'?(lang==='ar'?'ثابت':'Fixed'):(lang==='ar'?'ساعات محددة':'Set Hours')}
                    </span>
                  </div>
                  <div style={{display:'flex',gap:'14px',fontSize:'12px',color:'#64748B',fontWeight:'600'}}>
                    {s.timeMode==='open'?<span>{lang==='ar'?'⏰ تسجيل حضور فقط':'⏰ Check-in only'}</span>:s.checkIn&&<span>⏰ {s.checkIn} → {s.checkOut}</span>}
                    {s.timeMode!=='open'&&<span>🕐 {s.hoursPerDay} {lang==='ar'?'ساعة':'hrs'}</span>}
                    <span>📅 {s.daysPerWeek} {lang==='ar'?'أيام':'days'}</span>
                ...
                {[{n:1,l:{ar:'المستفيدون',en:'Recipients'}},{n:2,l:{ar:'التأكيد',en:'Confirm'}}].map((s,i)=>(
                  <React.Fragment key={s.n}>
                    <div style={{display:'flex',alignItems:'center',gap:'7px'}}>
                      <div style={{width:'28px',height:'28px',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'13px',fontWeight:'800',
                    ...
                    {[
                      ...
                      return(
                        <button key={opt.val} type="button"
                          ...
                          style={{padding:'14px 8px',borderRadius:'12px',cursor:'pointer',fontFamily:'Cairo',display:'flex',flexDirection:'column',alignItems:'center',gap:'8px',
                        ...
                        {[{id:'academic',l:{ar:'أكاديمي',en:'Academic'}},{id:'admin',l:{ar:'إداري',en:'Admin'}}].map(t=>{
                          ...
                          return(
                            <button key={t.id} type="button" onClick={()=>setGrantForm(p=>({...p,empType:t.id,collegeId:'',deptIds:[],deptSelections:{}}))}
                              style={{padding:'10px',borderRadius:'9px',cursor:'pointer',fontFamily:'Cairo',fontSize:'13px',fontWeight:'700',
                          ...
                          {COLLEGES.map(c=>{
                            ...
                            return(
                              <button key={c.id} type="button" onClick={()=>setGrantForm(p=>({...p,collegeId:c.id,deptIds:[],deptSelections:{}}))}
                                style={{padding:'8px 16px',borderRadius:'8px',cursor:'pointer',fontFamily:'Cairo',fontSize:'13px',fontWeight:'700',
