...
function Permissions({ lang, user }) {
  ...
  const usedMins = requests.filter(r => r.type !== 'exceptional' && r.type !== 'nursing' && r.status !== 'rejected').reduce((s, r) => s + (r.duration || r.durationMinutes || 0), 0);
  ...
  function openModal(pt) {
    setSelectedType(pt);
    setForm({ date: new Date().toISOString().slice(0, 10), reason: '', duration: pt.duration || 60 });
    setFormError('');
    setModalOpen(true);
  }
  ...
  async function submitRequest() {
    ...
    if (selectedType.id === 'nursing' && !nursingType) {
      setFormError(lang === 'ar' ? 'يرجى اختيار نظام الرضاعة' : 'Select nursing system');
      return;
    }
    if (selectedType.id !== 'nursing' && selectedType.id !== 'exceptional' && !form.date) {
      setFormError(lang === 'ar' ? 'يرجى تحديد التاريخ' : 'Select date');
      return;
    }
    if (!form.reason.trim()) {
      setFormError(lang === 'ar' ? 'يرجى إدخال السبب' : 'Enter reason');
      return;
    }
                    ...
                    {[
                      ...
                      return (
                        <button key={opt.val} type="button" onClick={() => canSelect && setForm(p => ({ ...p, duration: opt.val }))}
                          style={{
                            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', padding: '12px 8px', borderRadius: '12px', cursor: canSelect ? 'pointer' : 'not-allowed',
                    ...
                    {[
                      ...
                      return (
                        <button key={opt.val} type="button" onClick={() => setNursingType(opt.val)}
                          style={{
                            padding: '14px', borderRadius: '12px', cursor: 'pointer', fontFamily: 'Cairo', fontSize: '13px', fontWeight: '700',
