...
function Field({label,error,children}){
  return(
    <div>
      ...
      {children}
      {error&&<div style={{fontSize:'12px',color:'#DC2626',marginTop:'4px',fontWeight:'600'}}>{error}</div>}
    </div>
  );
}
                  ...
                  {[{v:'male',l:{ar:'ذكر',en:'Male'}},{v:'female',l:{ar:'أنثى',en:'Female'}}].map(g=>{
                    ...
                    return(
                      <button key={g.v} onClick={()=>setForm(p=>({...p,gender:g.v}))}
                        style={{padding:'12px',border:sel?'none':'1.5px solid #E2E8F0',borderRadius:'12px',cursor:'pointer',fontFamily:'Cairo',fontSize:'14px',fontWeight:'700',
