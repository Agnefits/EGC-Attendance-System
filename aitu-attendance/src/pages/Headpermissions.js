...
export default function HeadPermissions({ lang, user }) {
  ...
  const [permissions, setPermissions] = useState([
    ...
    { id:'P4', employeeId:empIds[1]||empIds[0], type:'nursing',     duration:60,  date:'شهري',       reason:'رضاعة طبيعية',status:'pending'  },
  ...
  function approve(id) {
    setPermissions(p=>p.map(x=>x.id===id?{...x,status:'approved'}:x));
    showToast(lang==='ar'?'✓ تمت الموافقة على الإذن':'✓ Permission approved');
    closeModal();
  }

  function reject() {
    if (!rejectNote.trim()) return;
    setPermissions(p=>p.map(x=>x.id===modal.id?{...x,status:'rejected',rejectNote}:x));
    showToast(lang==='ar'?'تم رفض الإذن':'Permission rejected','error');
    closeModal();
  }
