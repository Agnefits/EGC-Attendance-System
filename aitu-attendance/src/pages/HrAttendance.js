...
export default function HrAttendance({ lang, readOnly = false }) {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [attData, empData, deptData, leaveData, permData] = await Promise.all([
  ...
  const empOptions = useMemo(() =>
    filterDept === 'all' ? employees : employees.filter(e => e.departmentId === filterDept || e.department === filterDept)
      ...
      <div style={{ display: 'flex', gap: '4px', background: 'white', padding: '5px', borderRadius: '14px', border: '1px solid #E8EDF5', width: 'fit-content', marginBottom: '18px', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            style={{
              display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 22px', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'Cairo', transition: 'all .18s',
