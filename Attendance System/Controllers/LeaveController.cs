    ...
    public class LeaveController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 20)
        {
            ...
            var requests = await query
                ...
                .Take(pageSize)
                .Select(l => new LeaveRequestListItemDto
                {
                    Id = l.Id,
        ...
        public async Task<IActionResult> CreateLeaveRequest([FromBody] CreateLeaveRequestDto dto)
        {
            ...
            if (dto.LeaveTypeId == "compensatory")
            {
                ...
                var workedSaturday = await _unitOfWork.AttendanceLogs.Query().AnyAsync(a =>
                ...
                var hasCompensatory = await _unitOfWork.LeaveRequests.Query().AnyAsync(l =>
            ...
            return Ok(new { success = true, message = "Leave request submitted successfully", data = new LeaveRequestResultDto { Id = request.Id, DaysCount = request.DaysCount, Status = request.Status } });
        }
        ...
        public async Task<IActionResult> ApproveLeaveRequest(string id, [FromBody] ApproveLeaveDto dto)
        {
            ...
            if (role == "Head")
            {
                var headDeptId = await _unitOfWork.Employees.Query().Where(e => e.Id == approverId).Select(e => e.DepartmentId).FirstOrDefaultAsync();
            ...
            return Ok(new { success = true, message = dto.Approved ? "Request approved" : "Request rejected", data = new LeaveRequestApprovalResultDto { Id = request.Id, Status = request.Status } });
        }
